...
create procedure CalculatePostLikes  (
   ...
   BEGIN
       SELECT COUNT(*) -- tinh tong so like
       INTO total_likes -- luu kq cua select vao bien 
       FROM likes
...
-- B1: khai bao bien nhan tham so OUT
SET @total_likes = 0;
-- B2:Gọi stored procedure với post cụ thể
CALL CalculatePostLikes  (1, @total_likes);
-- Bước 3️: Truy vấn giá trị tham số OUT
SELECT @total_like AS TONGSOLIKE;
